...
def _srgb_to_linear(srgb: npt.NDArray[np.floating]) -> npt.NDArray[np.floating]:
    """Remove sRGB gamma correction (IEC 61966-2-1)."""
    out = np.empty_like(srgb, dtype=float)
    low = srgb <= 0.04045
    out[low] = srgb[low] / 12.92
    out[~low] = ((srgb[~low] + 0.055) / 1.055) ** 2.4
    return out


def _linear_to_srgb(linear: npt.NDArray[np.floating]) -> npt.NDArray[np.floating]:
    """Apply sRGB gamma correction (IEC 61966-2-1)."""
    out = np.empty_like(linear, dtype=float)
    low = linear <= 0.0031308
    out[low] = linear[low] * 12.92
    out[~low] = 1.055 * linear[~low] ** (1 / 2.4) - 0.055
    return out


def _machado_matrix(cvd_type: str, severity: int) -> npt.NDArray[np.floating]:
    """Get the Machado et al. (2009) CVD simulation matrix.

    Interpolates linearly between pre-computed matrices at multiples of 10.
    ...
    low_matrix = np.asarray(_MACHADO_MATRICES[cvd_type][low])
    if severity == 100:
        return low_matrix
    high_matrix = np.asarray(_MACHADO_MATRICES[cvd_type][low + 10])
    return (1 - fraction / 10.0) * low_matrix + (fraction / 10.0) * high_matrix


# Machado et al. (2009) CVD simulation matrices, severity 0-100 in steps of 10.
_MACHADO_MATRICES: dict[str, dict[int, list[list[float]]]] = {
    "deuteranomaly": {
        0: [
...
# ITU-R BT.709 luminance coefficients (matches sRGB primaries)
_LUMINANCE_WEIGHTS = np.array([0.2126, 0.7152, 0.0722])
